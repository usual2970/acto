...
func key(pointTypeID string) string { return "ranking:" + pointTypeID }

func (r *RankingRepository) UpdateUserScore(ctx context.Context, pointTypeID, userID string, score int64) error {
	return r.client.ZAdd(ctx, key(pointTypeID), goRedis.Z{Member: userID, Score: float64(score)}).Err()
}

func (r *RankingRepository) GetTop(ctx context.Context, pointTypeID string, start, stop int64) ([]string, error) {
